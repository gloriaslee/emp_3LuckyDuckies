...
// ~~====== TESTING lINSEARCH FOR INDIVIDUAL CASES
    ...
    // The following methods record and display to the user the elapsed time
    // for a given number of searches (of each type) for random values in a fixed array
    ...
    public static String MultCasesWrapper(int trials, int arrLength){
      ...
      output += (results[0] + "\n");
      output += ("Elapsed time for linear search of " + trials + " newly generated random values: ");
      output += (results[1] + "\n");
      return output;
    }
